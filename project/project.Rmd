...
We have also found the top 10 figures with the highest popularity index score. They are:
1 Aristotle
2	Plato
3	Jesus Christ
4	Socrates	
...
panth_mod %>%
  ...
  geom_boxplot() + 
  coord_flip() +
  labs(title = "Distribution of Average Page Views", y = "Page Views", 
       x = "Continent", subtitle = "by Continent")

panth_mod %>%
  filter(continent == "South America", average_views > 600000) %>%
  select(rank, full_name, country, continent, average_views)

panth_mod %>%
  filter(continent == "Oceania", average_views > 750000) %>%
  select(rank, full_name, country, continent, average_views)

panth_mod %>%
  filter(continent == "North America", average_views > 1500000) %>%
  select(rank, full_name, country, continent, average_views) 

panth_mod %>%
  filter(continent == "Europe", average_views > 1000000) %>%
  select(rank, full_name, country, continent, average_views)

panth_mod %>%
  filter(continent == "Asia", average_views > 600000) %>%
  select(rank, full_name, country, continent, average_views)

panth_mod %>%
  filter(continent == "Africa", average_views > 600000) %>%
  select(rank, full_name, country, continent, average_views)
```
...
Lionel Messi (South America)
Chris Hemsworth (Oceania)
Kim Kardashian (North America)
Cristiano Ronaldo (Europe)
Manny Pacquiáo (Asia)
Freddy Mercury (Africa)
